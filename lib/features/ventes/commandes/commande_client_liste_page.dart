...
class CommandeClientListePage extends ConsumerStatefulWidget {
  const CommandeClientListePage({super.key});

  @override
  ConsumerState<CommandeClientListePage> createState() => _CommandeClientListePageState();
}

class _CommandeClientListePageState extends ConsumerState<CommandeClientListePage> {
  ...
  Future<void> _traiterSelection() async {
    ...
    final confirme = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Les brouillons seront supprimés, les commandes déjà validées seront annulées.\n'
    ...
    final echecs = await ref.read(commandeClientListeProvider.notifier).annulerOuSupprimerPlusieurs(ids);
    if (!mounted) return;
    setState(() {
